...
## [2026-05-20T16:14:26.211Z] /relic.solve — 008-html-spec-mode / 2026-05-20-base-html-design-header-dark-mode: fix comment termination + header + dark mode + design

Fixed: HTML comment block in base.html contained Mermaid-style --> arrow syntax that prematurely terminated the comment, causing example content to render as visible page content in all spec HTML files. Documentation moved to <template id="relic-docs"> element which is inert and immune to this class of bug. Added: sticky header with dark mode toggle (CSS custom properties, localStorage persistence) and navigation links (spec.md, plan.md, tasks.md) relative to the spec directory. Added relic-chip component for compact inline metadata tags. Improved CSS design system: custom properties throughout, max-width 900px centered layout, table striping, improved typography. Strengthened anti-transcription guidance in HTML step of all 7 prompt templates. Updated HtmlComponentContract.md: documents template-based docs approach, relic-chip component, dark mode CSS variable contract, and header nav link convention.

## [2026-05-20T16:21:23.105Z] /relic.clarify — 008-html-spec-mode: clarify — FR-21/22 corrected, FR-24-27/NFR-8 added, HtmlComponentContract.md updated

FR-21 corrected: component docs now stored in <template id=relic-docs> (not HTML comment — --> arrows in examples broke browser rendering). FR-22 corrected: spec/fix HTML files are self-contained (CSS/JS embedded inline); runtime external base.html reference design dropped. FR-24 added: sticky header with dark mode toggle and spec navigation links. FR-25 added: CSS custom property system for dark mode with localStorage persistence. FR-26 added: relic-chip component for compact inline metadata tags. FR-27 added: relic validate must permit spec-id.html in spec folders in html mode (blocked — intersection with specs 005 and 009 on validate.ts, OQ-6). NFR-8 added: anti-transcription rule for all HTML step prompt sections. HtmlComponentContract.md updated to document all new features.
...
## [2026-07-02T16:54:29.016Z] /relic.plan — 011: reframed skill-extraction → claude-plugin (ambient SDD); plan written

Spec 011 renamed to 011-claude-plugin and rewritten: delivery becomes a first-party Claude Code plugin (commands generated from templates/prompts + four authored ambient skills: knowledge-first, spec-detector, fix-pipeline, doc-keeper) replacing per-project .claude/commands copies; behaviour becomes ambient SDD with a graduated autonomy ladder (read silent / maintain automatic / structural auto-with-announce) governed by a config.json sdd knob (auto|suggest). Decisions D-1..D-4 confirmed with the project owner. ClaudePluginContract created; SkillExtractionContract superseded and retained as historical pointer. Cross-spec notice: ContextResultContract (003) and ProjectConfigDomain (008) will gain the additive sdd field at implementation time. Tasks intentionally deferred pending owner review of spec+plan.

## [2026-07-02T17:56:13.365Z] /relic.clarify — 011-claude-plugin: open questions resolved; CLI bootstrap + per-project install added

OQ-1 resolved: plugin.json/marketplace.json schemas, /relic:command namespacing, and per-project enablement keys (extraKnownMarketplaces + enabledPlugins) verified against code.claude.com/docs and recorded in ClaudePluginContract. OQ-2 resolved: ambient skills never touch spec HTML — surface frozen pending a separate HTML reshape spec (D-9). OQ-3 resolved: single spelling, /relic.command renamed to /relic:command everywhere in one pass. OQ-4 resolved: upgrade cleanup gated behind --clean for one release. New requirements per owner: FR-17 consent-gated CLI auto-install on first use and FR-18 /relic:setup onboarding command (D-8); plugin installed per project via committed .claude/settings.json (D-7).

## [2026-07-02T18:18:53.450Z] /relic.implement — 011-claude-plugin: implemented — plugin, ambient skills, sdd knob, engine transition

plugin/ directory with 12 commands generated from templates/prompts (build-plugin.ts, CI freshness check) + authored /relic:setup; repo-root marketplace.json (repo is its own marketplace); four ambient skills (knowledge-first, spec-detector, fix-pipeline, doc-keeper) with byte-identical shared guard/ladder blocks and consent-gated CLI bootstrap; config.json sdd knob surfaced in relic context; writeClaude rewritten to per-project plugin installation (settings permission + extraKnownMarketplaces + enabledPlugins, no command copies); relic upgrade --clean removes superseded relic.*.md copies; /relic. renamed to /relic: across templates and docs (123 occurrences); preamble gains Ambient SDD section. Cross-artifact mutations: ContextResultContract (003) and ProjectConfigDomain (008) amended additively with the sdd field. claude plugin validate --strict passes.

## [2026-07-02T18:47:01.393Z] manual (1.0.0 roadmap phase 5) — maintenance: roadmap phase 5 — hardening and docs refresh

Root bun test green for the first time: upgrade.test.ts mock.module("@relic/engines") leaked a no-op runAddEngine process-globally into the engines suite; replaced with the _runAddEngine injection seam. All 16 pre-existing typecheck errors fixed (toon-migrate regex capture guards, test-file index assertions, toon.test.ts ported from vitest import to bun:test); tsc --noEmit added to CI. CLAUDE.md rewritten for the plugin era (session.json, full command surface, snippets/skills/plugin, sdd ladder, html chrome model, external integration, testing conventions); stale Open Questions removed in favour of knowledge-layer pointers. Upgrade path verified e2e across all three engines.

## [2026-07-02T21:54:21.925Z] /relic.plan — 012-spec-viewer: spec + plan written — server-rendered views over MCP

Spec 012 replaces the per-file HTML application model with real infrastructure: .relic as backend, an embedded React viewer served by a per-project read-only localhost server (port via config.json viewer.port, lifecycle in gitignored viewer.json), MCP tools (view_spec/view_fix/list_views) shipped through the plugin .mcp.json, and spec/fix HTML reduced to semantic <relic-body> fragments where data components (tasks, artifacts, meta, changelog) are server-derived and can never be stale. Owner decisions D-1..D-7: local server + MCP delivery; semantic tags + derived components (no LLM-facing templating); React; tools-only MCP; per-project lazy server; read-only server invariant; file:// support dropped. Cross-spec notices: HtmlComponentContract (008) supersession pointer added; ProjectConfigDomain (008) will gain viewer.port; ContextResultContract (003) will gain the viewer field. html-sync will be retired by this spec. Tasks deferred pending owner review.

## [2026-07-02T23:30:05.535Z] /relic.clarify — 012-spec-viewer: toolchain clarified — Vite dev/build, Bun runtime, packages/viewer

Owner refinement recorded as D-8/D-9: packages/viewer is scaffolded with the Vite react template (bun create vite --template react --no-interactive) in its own packages folder; full React to avoid library-compatibility dead ends. Vite is the dev/build toolchain only (HMR dev server proxying /api to relic serve; vite build produces dist/ embedded via scripts/embed-viewer.ts) — users never run Vite; relic serve (Bun) ships the built app at /, /api/* JSON, and an SPA fallback so deep links and future pages are additive paths. FR-5/FR-5b updated; plan Phase 3 concretised.

## [2026-07-03T03:14:43.957Z] /relic.implement — 012-spec-viewer: implemented — viewer server, MCP tools, fragment migration

Phases 5-7 complete the spec: relic mcp (hand-rolled newline-delimited JSON-RPC stdio server; view_spec/view_fix/list_views ensure the viewer and return URLs; in-flight drain on stdin close) shipped via plugin .mcp.json; relic viewer-migrate converted all 5 spec HTMLs and 4 fix HTMLs in this repo to lint-clean <relic-body> fragments and removed .relic/base.html; relic html-sync retired (stub), mode html no longer writes chrome; html-mode snippet rewritten for fragment authoring with new viewer-components reference snippet (nested include); 7 server API tests + migrate tests + MCP smoke; relic validate fully clean. Cross-artifact mutations: ContextResultContract (003) gains the viewer field, ProjectConfigDomain (008) gains viewer.port, ClaudePluginContract (011) gains the .mcp.json component, HtmlComponentContract (008) supersession completed. ViewerContract confidence raised to high with implemented API/MCP shapes.

## [2026-07-16T01:31:50.807Z] manual — manual (PR #64 review) — knowledge docs realigned to fragment era

Copilot PR review resolution: preamble (template + instance, byte-identical) no longer tells agents spec HTML chrome is maintained by the retired relic html-sync — spec HTML is a <relic-body> fragment created by relic scaffold and rendered by relic serve. SpecFilesAllowlistRule html-mode exception restated in fragment terms. ScaffoldResultContract files_synced documented as vestigial (always [], retained for output-shape stability). Docs collateral from the /relic. -> /relic: mass rename fixed: relic.js / relic.exe artifact paths in distribution.md and implementation.md had become relic:js / relic:exe.

## [2026-07-16T01:47:49.296Z] manual — manual (viewer dogfooding) — three viewer bugs fixed: markdown hang, ensure-server race, context port

Dogfooding relic serve + relic mcp on this repo surfaced three code-bugs, all fixed with fix documents 2026-07-16-viewer-markdown-infinite-loop and 2026-07-16-viewer-server-race-context-port: (1) markdownToHtml infinite-looped on paragraph lines starting with inline code — the guard excluded what no block branch consumed; a dispatch-mirroring isBlockStart() plus a progress fallback fixes it (all 36 repo markdown files render in 6ms). (2) concurrent MCP tool calls raced ensureServer into spawning duplicate servers on auto-incremented ports with inconsistent viewer.json; now serialized behind one in-flight promise. (3) relic context probed only the configured viewer port, reporting running:false for healthy auto-incremented servers; it now prefers the viewer.json runtime port. All verified live from clean state.
