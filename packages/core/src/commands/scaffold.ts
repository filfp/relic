...
import { syncSpecHtml } from "./html-sync.ts";
...
export async function runScaffold(options: ScaffoldOptions): Promise<void> {
  ...
  // when it already exists, re-base its chrome onto the current template so
  // files generated by older Relic versions pick up template fixes
  ...
  if (readMode(relicDir) === "html") {
    ...
    if (!fileExists(htmlDest)) {
      ...
      // embed the reader sources right away (part of creation, not a sync)
      syncSpecHtml(specsDir, specId);
    } else if (syncSpecHtml(specsDir, specId) === "synced") {
      filesSynced.push(`${specId}.html`);
    }
  }
